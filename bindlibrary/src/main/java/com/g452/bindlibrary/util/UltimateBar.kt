...
class UltimateBar {
    ...
    fun apply() {
        when (type) {
            TYPE_COLOR -> {
                colorBuilder?.let {
                    setColorBar(it.statusColor, it.statusDepth, it.applyNav, it.navColor, it.navDepth)
                }
            }
            TYPE_TRANSPARENT -> {
                transparentBuilder?.let {
                    setTransparentBar(it.statusColor, it.statusAlpha, it.applyNav, it.navColor, it.navAlpha)
                }
            }
            TYPE_IMMERSION -> {
                immersionBuilder?.let {
                    setTransparentBar(Color.TRANSPARENT, 0, it.applyNav, Color.TRANSPARENT, 0)
                }
            }
            TYPE_DRAWER -> {
                drawerBuilder?.let {
                    setColorBarForDrawer(it.statusColor, it.statusDepth, it.applyNav, it.navColor, it.navDepth)
                }
            }
            TYPE_HIDE -> {
                hideBuilder?.let {
    ...
    fun setColorBar(
        @ColorInt statusColor: Int, statusDepth: Int, @ColorInt navColor: Int, navDepth: Int
    ) {
    ...
    private fun setHideBar(applyNav: Boolean) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            ...
            if (applyNav) {
                option = (option or View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION or View.SYSTEM_UI_FLAG_HIDE_NAVIGATION)
            }
    ...
    private fun setColorBar(
        @ColorInt statusColor: Int, statusDepth: Int, applyNav: Boolean, @ColorInt navColor: Int, navDepth: Int
    ) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            val finalStatusColor = if (realStatusDepth == 0) statusColor else calculateColor(statusColor, realStatusDepth)
            window.statusBarColor = finalStatusColor
            if (applyNav) {
                val realNavDepth = limitDepthOrAlpha(navDepth)
                val finalNavColor = if (realNavDepth == 0) navColor else calculateColor(navColor, realNavDepth)
    ...
    private fun setTransparentBar(
        @ColorInt statusColor: Int, statusAlpha: Int, applyNav: Boolean, @ColorInt navColor: Int, navAlpha: Int
    ) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            val window = activity.window
            ...
            val finalStatusColor = if (statusColor == 0) Color.TRANSPARENT else Color.argb(
                limitDepthOrAlpha(statusAlpha), Color.red(statusColor), Color.green(statusColor), Color.blue(statusColor)
            )
            window.statusBarColor = finalStatusColor
            if (applyNav) {
                ...
                val finalNavColor = if (navColor == 0) Color.TRANSPARENT else Color.argb(
                    limitDepthOrAlpha(navAlpha), Color.red(navColor), Color.green(navColor), Color.blue(navColor)
                )
                window.navigationBarColor = finalNavColor
            }
            decorView.systemUiVisibility = option
